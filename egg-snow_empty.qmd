...
nest_species <- eggs_sum %>% 
  left_join(nest_data, by = 'Nest_ID')
...
nest_species_avg <- nest_species %>%
  group_by(Species) %>% 
  summarise(avg_egg_n = mean(n_egg))
nest_species_avg
```
...
nest_species_avg %>% 
  left_join(species_data, 
            join_by(Species == Code)) # Called different columns in the dif dfs 
```
...
snowcover_data_fixed <- snowcover_data_fixed %>% 
  mutate(Date = ifelse(Date == "8&9 june 06", "08-June-06", Date))
